...
<html>
  <head>
	...
	<link href="<%=path %>/css/manage.css" rel="stylesheet" type="text/css">
  ...
  <table cellSpacing=1 cellPadding=0 width="100%" bgColor=#d4d0c8 border=0>
    <tbody>		
		...
		<tr class=toptr3>
			...
			<td><s:a href="viewTbipc.action?tbipc.id=%{#pl.id}" >查看</s:a>
   			</td>   
   						
   			<td><s:a href="updatePTbipc.action?tbipc.id=%{#pl.id}">修改</s:a>
   			
   			</td>
   			<td><s:a href="deleteTbipc.action?tbipc.id=%{#pl.id}" onclick="return del();">删除</s:a>
...
<table class=table_tool cellSpacing=0 cellPadding=3 border=0 >
            ...
            <s:else> 
					<a href="listTbipc.action?page=1" onClick="firstPage()"><img alt=首页 src="<%=request.getContextPath() %>/image/page_first.gif" border=0></a>
				
					<a href="listTbipc.action?page=<s:property value="%{pageBean.currentPage-1}"/>"><img alt=上页 src="<%=request.getContextPath() %>/image/page_prio.gif" border=0></a>
			</s:else> 
			<s:if test="%{pageBean.currentPage != pageBean.totalPage}"> 
					<a href="listTbipc.action?page=<s:property value="%{pageBean.currentPage+1}"/>"><img alt=下页 src="<%=request.getContextPath() %>/image/page_next.gif" border=0></a>
				
					<a href="listTbipc.action?page=<s:property value="%{pageBean.totalPage}"/>"><img alt=尾页 src="<%=request.getContextPath() %>/image/page_last.gif" border=0></a>
			</s:if>  
             <s:else>  
