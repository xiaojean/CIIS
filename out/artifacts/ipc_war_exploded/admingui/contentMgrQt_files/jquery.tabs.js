    ...
    return this.each(function() {
        ...
        if (settings.remote) {
            tabs.each(function() {
                ...
                this.href = hash;
                $('<div id="' + id + '" class="' + settings.containerClass + '"><table></table></div>').appendTo(container);
                $(this).bind('loadRemoteTab', function(e, callback) {
                    var $$ = $(this).addClass(settings.loadingClass), span = $('span', this)[0], tabTitle = span.innerHTML;
                    if (settings.spinner) {
                        // TODO if spinner is image
                        span.innerHTML = '' + settings.spinner + ''; // WARNING: html(...) crashes Safari with jQuery 1.1.2
                    }
                    setTimeout(function() { // Timeout is again required in IE, "wait" for id being restored
                       $.getJSON(url, function(data){
                       		$(hash).empty();
                       		if(data.error){
                       			$(hash).append(data.error);
                       		}
                       		else{
                       			$(hash).append('<table></table>');
                       			switch($(hash).parent().attr('id')){
	                   				case 'task':
	    								$(hash+' table:first-child').append('<tr><th class="title">标题</th><th class="time">时间</th><th class="type">类型</th></tr>');
	     								$.each(data.data, function(i,item){
	    									$(hash+' table:first-child').append('<tr><td><a href="'+item.titleurl+'" title="'+item.title+'">'+item.title+'</a></td><td>'+item.createTime+'</td><td>'+item.trans+'</td></tr>');
	    								});
	    	                            $(hash).append('<a href="'+data.more+'" class="more" title="更多"></a>');
	                   					break;
	                   				case 'news':
	                   					$(hash+' table:first-child').append('<tr><th class="title"></th><th class="time"></th></tr>');
	     								$.each(data.data, function(i,item){
	    									$(hash+' table:first-child').append('<tr><td><a href="'+item.titleurl+'" title="'+item.title+'">'+item.title+'</a></td><td>'+item.createTime+'</td></tr>');
	    								});
	    	                            $(hash).append('<a href="'+data.more+'" class="more" title="更多"></a>');
	                   					break;
	                   				case 'notice':
	                   				...
	                   				case 'pmailbox':
	    	                            ...
	    	                            $(hash).append('<a href="'+data.more+'" class="more" title="更多"></a>');
	                   					break;
	                   				case 'leader':
	    								$(hash+' table:first-child').append('<tr><th class="title">标题</th><th class="time">时间</th><th class="type">人员</th></tr>');
	     								$.each(data.data, function(i,item){
	    									$(hash+' table:first-child').append('<tr><td><a href="'+item.titleurl+'" title="'+item.title+'">'+item.title+'</a></td><td>'+item.createTime+'</td><td>'+item.trans+'</td></tr>');
	    								});
	    	                            $(hash).append('<a href="'+data.more+'" class="more" title="更多"></a>');
